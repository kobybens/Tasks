import { api, state, loadUsers, render, toast } from '../app.js';
import { esc, PALETTE } from '../lib.js';
  ...
  return `
        ...
        ${state.users
          ...
          <div class="user-row" data-id="${u.id}">
            ...
            <span class="ops">
              ...
              <button class="btn" data-admin="color" data-id="${u.id}">Color</button>
    ...
    <div class="card">
      ...
      <form class="form" id="add-user-form">
        ...
        <div class="field"><label for="au-pw">Password</label><input id="au-pw" name="password" type="text" required minlength="4" autocomplete="off" placeholder="At least 4 characters" /></div>
        ...
        <label style="display:flex;gap:8px;align-items:center;min-height:44px"><input type="checkbox" name="is_admin" /> Can manage users (admin)</label>
  ...
  root.querySelector('.user-list')?.addEventListener('click', async (e) => {
    ...
    try {
      switch (btn.dataset.admin) {
        case 'reset': {
          const pw = prompt(`New password for ${u.display_name} (at least 4 characters):`);
          if (pw == null) return;
